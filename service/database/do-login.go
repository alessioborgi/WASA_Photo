...
func (db *appdbimpl) DoLogin(username string) (string, error) {
	...
	if errors.Is(errUserPresence, ErrUserDoesNotExist) {
		...
		_, errCretion := db.c.Exec(`INSERT INTO Users (fixedUsername, uuid, username, photoProfile, biography, dateOfCreation, numberOfPhotos, numberFollowers, numberFollowing, name, surname, dateOfBirth, email, nationality, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
