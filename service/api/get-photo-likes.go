...
func (rt *_router) getPhotoLikes(w http.ResponseWriter, r *http.Request, ps httprouter.Params, ctx reqcontext.RequestContext) {
	...
	log.Println("The User that owns the Photos I am going to get it likes of is: ", username.Name)

	if username.Name == "" {

		// If the username is empty, there is a bad request.
		w.WriteHeader(http.StatusBadRequest)
		log.Println("Err: The Photo Like List cannot be acquired because it has received an Empty username.")
		return
	}
	...
	log.Println("The Photoid that for which we want to know its List of Likes is: ", photoid)
	...
	users, err := rt.db.GetPhotoLikes(username.Name, photoid, authorization_token)
	...
	if errors.Is(err, database.ErrUserDoesNotExist) {
		...
		// In this case, we have that the Photo that was requested to get its likes, is not in the WASAPhoto Platform.
		w.WriteHeader(http.StatusBadRequest)
		log.Println("Err: The Photo for which we would like to know its Like Lists, is not a WASAPhoto Photo.")
		return
		...
		log.Println("We can correctly return the list of likes(UsernameLikers).")
